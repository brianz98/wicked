...
__all__ = ["string_to_expr", "gen_op","compile_einsum", "compile_sigma_vector"]
...
def compile_sigma_vector(equation, bra_name='bra'):
    """
    ...
    for t in rhs.tensors():
        if (t.label() == bra_name):
            lhs_tensor_label = "[\"" + ''.join([str(_)[0] for _ in t.upper()]) + ''.join([str(_)[0] for _ in t.lower()]) + "\"]"
            bra_index = _get_unique_tensor_indices(t, unused_indices, index_dict)
        else:
            tensor_label_string += t.label() + "[\"" + ''.join([str(_)[0] for _ in t.upper()]) + ''.join([str(_)[0] for _ in t.lower()]) + "\"],"
            index_string += _get_unique_tensor_indices(t, unused_indices, index_dict)
            index_string += ','
